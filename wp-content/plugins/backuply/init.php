...
function backuply_load_plugin(){
	...
	if(defined('BACKUPLY_PRO')) {
		...
		if(!empty($holiday_time) && $holiday_time > 0 && isset($_GET['page']) && $_GET['page'] === 'backuply' && in_array($time, $days)){
			add_action('admin_notices', 'backuply_holiday_promo');
			$is_holiday = true;
		}
		
		// Are we to disable the holiday promo for 48 hours
		if(isset($_GET['backuply_holiday_promo']) && (int)$_GET['backuply_holiday_promo'] == 0 ){
			if(!wp_verify_nonce(backuply_optreq('security'), 'backuply_nonce')) {
		...
		if(empty($is_holiday) && !empty($promo_time) && $promo_time > 0 && $promo_time < (time() - (7 * 86400))){
